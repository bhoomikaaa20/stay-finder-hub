...
function AdminHotels() {
  ...
  const save = async () => {
    ...
    const url = editing
      ? `http://localhost:5000/api/admin/hotels/${editing._id}`
  ...
  const remove = async (id: string) => {
    await fetch(`http://localhost:5000/api/admin/hotels/${id}`, {
      method: "DELETE",
    });
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <Field label="Min price" type="number" value={String(form.price_min)} onChange={(v) => setForm({ ...form, price_min: Number(v) })} />
              <Field label="Max price" type="number" value={String(form.price_max)} onChange={(v) => setForm({ ...form, price_max: Number(v) })} />
              <Field label="Rating" type="number" value={String(form.rating)} onChange={(v) => setForm({ ...form, rating: Number(v) })} />
            </div>
